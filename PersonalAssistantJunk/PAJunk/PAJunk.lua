...
-- -----------------------------------------------
-- Sell all Companions items (to remove when ZoS will allow set companion items as junk)
local function SellCompanionItems()
     local PAJunkSavedVars = PAJ.SavedVars
     if PAJunkSavedVars.Miscellaneous.autoMarkCompanionItemsQualityThreshold ~= PAC.ITEM_QUALITY.DISABLED then
		 local bagpack = SHARED_INVENTORY.bagCache[BAG_BACKPACK]
		 for _, slotData in pairs(bagpack) do
		     if PAHF.isItemForCompanion(slotData.bagId, slotData.slotIndex) then
			     local itemQuality = GetItemFunctionalQuality(slotData.bagId, slotData.slotIndex) 
			     if itemQuality <= PAJunkSavedVars.Miscellaneous.autoMarkCompanionItemsQualityThreshold then
				     local quantity,_ = GetSlotStackSize(slotData.bagId, slotData.slotIndex) or 1
				     SellInventoryItem(slotData.bagId, slotData.slotIndex, quantity)
			     end
		     end
		 end   
     end	 
...
local function CheckAndLaunderItem(bagId, slotIndex, quantity) 
    local totalLaunders, laundersUsed = GetFenceLaunderTransactionInfo()
	local remainingLaunders = totalLaunders - laundersUsed
	if totalLaunders == 0 or remainingLaunders <= 0 then
	    return
	end
	
	if quantity > remainingLaunders then
	    quantity = remainingLaunders
	end
	
	local itemLink = GetItemLink(bagId, slotIndex, LINK_STYLE_BRACKETS)
	
	local totalPrice = GetItemLaunderPrice(bagId, slotIndex) * quantity
	local money = PAHF.getFormattedCurrency(totalPrice)
	if GetCurrentMoney() >= totalPrice then
	    LaunderItem(bagId, slotIndex, quantity)
		PAJ.println(SI_PA_CHAT_JUNK_AUTO_LAUNDERED, quantity, itemLink, money)
	end
end

-- auto launder
local function AutoLaunder()
	 ...
	 local totalLaunders, laundersUsed = GetFenceLaunderTransactionInfo()
	 if totalLaunders == 0 or totalLaunders - laundersUsed <= 0 then
		 return
	 end

     local PAJunkSavedVars = PAJ.SavedVars
     if PAJunkSavedVars.autoLaunderEnabled and AreAnyItemsStolen(BAG_BACKPACK) then

		 local bagpack = SHARED_INVENTORY.bagCache[BAG_BACKPACK]
		 for _, slotData in pairs(bagpack) do
            if IsItemStolen(slotData.bagId, slotData.slotIndex) then
			   ...
			   if itemId == 30357 and PAJunkSavedVars.AutoLaunder.autoLaunderLockPicks then 
			        CheckAndLaunderItem(slotData.bagId, slotData.slotIndex, quantity)  
			   elseif (itemId == 71779 or itemId == 73753 or itemId == 73754) and PAJunkSavedVars.AutoLaunder.autoLaunderEdicts then
			        CheckAndLaunderItem(slotData.bagId, slotData.slotIndex, quantity)
			   elseif ItemType == ITEMTYPE_SOUL_GEM and PAJunkSavedVars.AutoLaunder.autoLaunderSoulGems then
			        CheckAndLaunderItem(slotData.bagId, slotData.slotIndex, quantity)
			   elseif (ItemType == ITEMTYPE_INGREDIENT or ItemType == ITEMTYPE_SPICE or ItemType == ITEMTYPE_FLAVORING) and PAJunkSavedVars.AutoLaunder.autoLaunderIngredients then
			        CheckAndLaunderItem(slotData.bagId, slotData.slotIndex, quantity)
			   elseif (ItemType == ITEMTYPE_BLACKSMITHING_MATERIAL or ItemType == ITEMTYPE_BLACKSMITHING_RAW_MATERIAL or ItemType == ITEMTYPE_CLOTHIER_MATERIAL or
					   ItemType == ITEMTYPE_CLOTHIER_RAW_MATERIAL or ItemType == ITEMTYPE_JEWELRYCRAFTING_RAW_MATERIAL or ItemType == ITEMTYPE_JEWELRYCRAFTING_MATERIAL or
					   ItemType == ITEMTYPE_RAW_MATERIAL or ItemType == ITEMTYPE_STYLE_MATERIAL or ItemType == ITEMTYPE_WOODWORKING_MATERIAL or
					   ItemType == ITEMTYPE_WOODWORKING_RAW_MATERIAL) and PAJunkSavedVars.AutoLaunder.autoLaunderMaterials then
			        CheckAndLaunderItem(slotData.bagId, slotData.slotIndex, quantity)
			   elseif (ItemType == ITEMTYPE_WOODWORKING_BOOSTER or ItemType == ITEMTYPE_BLACKSMITHING_BOOSTER or ItemType == ITEMTYPE_CLOTHIER_BOOSTER or
					   ItemType == ITEMTYPE_ENCHANTMENT_BOOSTER or ItemType == ITEMTYPE_JEWELRYCRAFTING_BOOSTER) and PAJunkSavedVars.AutoLaunder.autoLaunderCraftingBoosters then
			        CheckAndLaunderItem(slotData.bagId, slotData.slotIndex, quantity)
			   elseif (ItemType == ITEMTYPE_ENCHANTING_RUNE_ASPECT or ItemType == ITEMTYPE_ENCHANTING_RUNE_ESSENCE or ItemType == ITEMTYPE_ENCHANTING_RUNE_POTENCY) and
			           PAJunkSavedVars.AutoLaunder.autoLaunderEnchantingRunes then
			        CheckAndLaunderItem(slotData.bagId, slotData.slotIndex, quantity)
			   elseif (ItemType == ITEMTYPE_GLYPH_ARMOR or ItemType == ITEMTYPE_GLYPH_JEWELRY or ItemType == ITEMTYPE_GLYPH_WEAPON) and
			           PAJunkSavedVars.AutoLaunder.autoLaunderGlyphs then
			        CheckAndLaunderItem(slotData.bagId, slotData.slotIndex, quantity)
			   elseif ItemType == ITEMTYPE_FURNISHING and PAJunkSavedVars.AutoLaunder.autoLaunderFurnishing then
			        CheckAndLaunderItem(slotData.bagId, slotData.slotIndex, quantity)
...
local function OnInventorySingleSlotUpdate(eventCode, bagId, slotIndex, isNewItem, itemSoundCategory, inventoryUpdateReason, stackCountChange)
    if PAJProfileManager.hasActiveProfile() then
        ...
        if (not isCrafted or not PAJunkSavedVars.ignoreCraftedItems) and
                ...
                (PA.WindowStates.isTransmuteStationClosed) and not (IsItemLinkSetCollectionPiece(itemLink) and not IsItemSetCollectionPieceUnlocked(GetItemLinkItemId(itemLink))) then

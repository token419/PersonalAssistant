...
local function _findFirstEmptySlotAndTargetBagFromSourceBag(sourceBagId)
    ...
    if sourceBagId == BAG_BACKPACK then
		...
		if PAB.currentBankBag and IsHouseBankBag(PAB.currentBankBag) then
		    targetBagId = PAB.currentBankBag
            targetSlotIndex = FindFirstEmptySlotInBag(targetBagId)
		end

    elseif sourceBagId == BAG_BANK or (sourceBagId == BAG_SUBSCRIBER_BANK and IsESOPlusSubscriber()) or IsHouseBankBag(sourceBagId) then
...
local function _stackInTargetBagAndPopulateNotMovedItemsTable(fromBagCache, toBagCache, newStacksAllowed, notMovedItemsTable, overruleStackToMove) -- hereherehere
	for _, fromBagItemData in pairs(fromBagCache) do
        ...
        for toBagCacheIndex, toBagItemData in pairs(toBagCache) do
            if _isSameItem(fromBagItemData, toBagItemData) then
                ...
                local targetStack = toBagItemData.stackCount -- cannot use [GetSlotStackSize] because it does not reflect changes after the bagCache is created
...
local function _getBagCacheBasedOnItemOperator(backpackBagCache, bankBagCache, operator)
	if operator >= PAC.OPERATOR.BANK_EQUAL then
        -- operator 6 - 60 => rule set for the bank & housebags
...
local function _getBagIdFromOperator(bankingOperator)
    ...
    elseif bankingOperator < 21 then
	    return BAG_HOUSE_BANK_TWO
    elseif bankingOperator < 26 then
	    return BAG_HOUSE_BANK_THREE
    elseif bankingOperator < 31 then
	    return BAG_HOUSE_BANK_FOUR
    elseif bankingOperator < 36 then
	    return BAG_HOUSE_BANK_FIVE
    elseif bankingOperator < 41 then
	    return BAG_HOUSE_BANK_SIX
    elseif bankingOperator < 46 then
	    return BAG_HOUSE_BANK_SEVEN
    elseif bankingOperator < 51 then
	    return BAG_HOUSE_BANK_EIGHT
    elseif bankingOperator < 56 then
	    return BAG_HOUSE_BANK_NINE
    else
	    return BAG_HOUSE_BANK_TEN		
    end
end

local function _isOperatorSetoToEqualOrLess(operator) 
    local modifiedOperator = operator 
    while modifiedOperator > 5 do
	    modifiedOperator = modifiedOperator - 5
	end
	
    if modifiedOperator == PAC.OPERATOR.BACKPACK_EQUAL then return true end
    if modifiedOperator == PAC.OPERATOR.BACKPACK_LESSTHAN then return true end
    if modifiedOperator == PAC.OPERATOR.BACKPACK_LESSTHANOREQUAL then return true end
...
local function _isOperatorSetToEqualOrGreater(operator)
    ...
    if modifiedOperator == PAC.OPERATOR.BACKPACK_EQUAL then return true end
    if modifiedOperator == PAC.OPERATOR.BACKPACK_GREATERTHAN then return true end
    if modifiedOperator == PAC.OPERATOR.BACKPACK_GREATERTHANOREQUAL then return true end
...
local function doIndividualItemTransactions(individualItems, backpackBagCache, bankBagCache, isCustomPAItemIdList)
    ...
    for itemId, customItemData in pairs(individualItems) do
		...
		-- verify we are on the right bank / house bag 
		if PAB.getBagIdFromOperator(operator) == PAB.currentBankBag or (PAB.getBagIdFromOperator(operator) == BAG_BACKPACK and PAB.currentBankBag == BAG_BANK) then
...
PA.Banking.getBagIdFromOperator = _getBagIdFromOperator
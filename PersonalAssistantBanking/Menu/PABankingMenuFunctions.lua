...
--------------------------------------------------------------------------
-- PABanking   Advanced.specializedItemType      advancedItemTypeStylePageMoveSetting -- hereherehere
---------------------------------
local function getPABankingAdvancedStylePageMoveSetting(isKnown)
    if isNoProfileSelected() then return end
    if isKnown then
        return PAB.SavedVars.Advanced.StylePages[SPECIALIZED_ITEMTYPE_COLLECTIBLE_STYLE_PAGE].Known
    else
        return PAB.SavedVars.Advanced.StylePages[SPECIALIZED_ITEMTYPE_COLLECTIBLE_STYLE_PAGE].Unknown
    end
end

local function setPABankingAdvancedStylePageMoveSetting(value, isKnown)
    if isNoProfileSelected() then return end
    if isKnown then
        PAB.SavedVars.Advanced.StylePages[SPECIALIZED_ITEMTYPE_COLLECTIBLE_STYLE_PAGE].Known = value
    else
        PAB.SavedVars.Advanced.StylePages[SPECIALIZED_ITEMTYPE_COLLECTIBLE_STYLE_PAGE].Unknown = value
    end
end
...
---------------------------------
local function isAdvancedStylePageCraftingTypesDisabledOrAllStylePageMoveModeIgnore()
    if isDisabled({"Advanced", "advancedItemsEnabled"}) then return true end

    for _, specializedItemType in ipairs(PAC.BANKING_ADVANCED.STYLE_PAGES) do
        if PAB.SavedVars.Advanced.StylePages[SPECIALIZED_ITEMTYPE_COLLECTIBLE_STYLE_PAGE].Known ~= PAC.OPERATOR.NONE or PAB.SavedVars.Advanced.StylePages[SPECIALIZED_ITEMTYPE_COLLECTIBLE_STYLE_PAGE].Unknown ~= PAC.OPERATOR.NONE then return false end
    end
    -- if there was no 'false' returned until here; then return true
    return true
end
...
local function setPABankingAdvancedItemTypeMoveAllSettings(value)
    ...
    for specializedItemType, _ in pairs(PAB.SavedVars.Advanced.StylePages) do
        PAB.SavedVars.Advanced.StylePages[specializedItemType].Known = value
		PAB.SavedVars.Advanced.StylePages[specializedItemType].Unknown = value
    end
...
local PABankingMenuFunctions = {
	...
	isScribingItemsTransactionMenuDisabled = function() return isAdvancedItemsDisabledOrAllSpecializedtemTypesMoveModeIgnore(PAC.BANKING_ADVANCED.SPECIALIZED.SCRIBING) end,

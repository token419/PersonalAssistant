...
local PABAdvancedStylePageSubmenuTable = setmetatable({}, { __index = table })
...
local PABAdvancedScribingSubmenuTable = setmetatable({}, { __index = table })
...
local function _createPABankingMenu()
    ...
    PABankingOptionsTable:insert({
        ...
        icon = PAC.ICONS.ITEMS.STYLE_PAGE.PATH,
        controls = PABAdvancedStylePageSubmenuTable,
        disabledLabel = PABMenuFunctions.isStylePageTransactionMenuDisabled,
    })
    ...
    PABankingOptionsTable:insert({ 
        ...
        iconTextureCoords = PAC.ICONS.TEXTURE_COORDS.MEDIUM,
        controls = PABAdvancedScribingSubmenuTable,
        disabledLabel = PABMenuFunctions.isScribingItemsTransactionMenuDisabled,
    })
...
local function _createPABAdvancedStylePageSubmenuTable() 
    for _, specializedItemType in pairs(PAC.BANKING_ADVANCED.STYLE_PAGES) do
        ...
        PABAdvancedStylePageSubmenuTable:insert({
            type = "dropdown",
            name = GetString(SI_PA_MENU_BANKING_ADVANCED_UNKNOWN_STYLE_PAGE),
            choices = PABMenuChoices.itemMoveMode,
            choicesValues = PABMenuChoicesValues.itemMoveMode,
            getFunc = function() return PABMenuFunctions.getPABankingAdvancedStylePageMoveSetting(false) end,
            setFunc = function(value) PABMenuFunctions.setPABankingAdvancedStylePageMoveSetting(value, false) end,
            disabled = PABMenuFunctions.isAdvancedItemsDisabled,
            default = PABMenuDefaults.Advanced.StylePages[specializedItemType].Unknown,
        })
    end
end
...
local function createOptions()
	...
	_createPABAdvancedStylePageSubmenuTable()

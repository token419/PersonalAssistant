...
local function _hasItemIconChecksPassed(itemType, specializedItemType, itemFilterType, itemUseType, itemLink)
    if PA.Loot.SavedVars.ItemIcons.itemIconsEnabled then
        if itemType == ITEMTYPE_RECIPE or itemType == ITEMTYPE_RACIAL_STYLE_MOTIF or
            itemFilterType == ITEMFILTERTYPE_ARMOR or itemFilterType == ITEMFILTERTYPE_WEAPONS or itemFilterType == ITEMFILTERTYPE_JEWELRY or
                specializedItemType == SPECIALIZED_ITEMTYPE_CONTAINER_STYLE_PAGE or specializedItemType == SPECIALIZED_ITEMTYPE_COLLECTIBLE_STYLE_PAGE or specializedItemType == SPECIALIZED_ITEMTYPE_CONTAINER or
				(itemUseType == ITEM_USE_TYPE_CRAFTED_ABILITY_SCRIPT or itemUseType == ITEM_USE_TYPE_CRAFTED_ABILITY) then
...
local function _addItemKnownOrUnknownVisuals(parentControl, itemLink, hookType)
    ...
    if not _hasItemIconChecksPassed(itemType, specializedItemType, itemFilterType, itemUseType, itemLink) then
        return
    end
